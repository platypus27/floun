...
Floun processes the active tab URL locally to build scan target metadata, minimizing it to the tab origin before the target is sent to the background worker. This removes paths, credentials, query strings, and fragments. It also processes the active tab hostname, visible page metadata, same-origin script text, sanitized same-origin script locations, and browser-visible token candidates that match local heuristics. Token evidence is redacted before display in reports.
