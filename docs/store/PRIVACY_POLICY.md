...
Floun may process the active tab URL locally to build scan target metadata. Target URL metadata strips credentials, query strings, and fragments before it is sent to the background worker. Floun may also process the active tab hostname, page metadata, same-origin JavaScript text, TLS metadata, certificate metadata, and browser-visible token candidates that match local session-token heuristics.
