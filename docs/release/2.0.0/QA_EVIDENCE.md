...
Status: scripted release-prep, hardening audit checks, Chrome for Testing popup-flow QA, and PDF redaction QA passed on June 8, 2026. The release remains local only and is not pushed, tagged, uploaded, or published.
...
- Generated by: `npm run package:extension` during Phase 7 Chrome QA hardening verification
- SHA-256: `e918fec5458a447a2902152f1d75cb8ea906fef1d41c633b28dcac8f4bff31b7`
- Alias SHA-256: `e918fec5458a447a2902152f1d75cb8ea906fef1d41c633b28dcac8f4bff31b7`
- Size bytes: `317502`
...
- `index.html`
- `background.js`
- `assets/ai-handler-CdYEzpv0.js`
- `assets/index-lu6xBpGI.css`
- `assets/index-DoTA1TRt.js`
- `assets/pdfService-D3pyjaWx.js`
...
| deterministic packaging check | Pass | Two consecutive package runs produced matching SHA-256: `e918fec5458a447a2902152f1d75cb8ea906fef1d41c633b28dcac8f4bff31b7`. |
...
| `npm run qa:chrome:flows` with Chrome for Testing | Pass | Verified local fixture scan, known HTTPS scan, HTTP certificate warning, unsupported page handling, PDF download, and raw-token redaction through Chrome CDP `Extensions.triggerAction` popup flow. |
| `npm test` | Pass | 29 test files, 107 tests. |
...
- Repeatable helper: `npm run qa:extension:load` passed when `FLOUN_CHROME_BIN` pointed at Chrome for Testing `149.0.7827.54`, verifying Floun from the loaded extension runtime manifest. `npm run qa:chrome:flows` then drove the real extension action with CDP `Extensions.triggerAction`, scanned tab targets, generated a PDF report, and checked downloaded PDF bytes for raw fixture tokens.
- Branded Chrome note: the helper is expected to fail on branded Google Chrome 148 with an actionable message because official Chrome removed command-line extension loading support.
...
| Scan `http://127.0.0.1:4174/crypto-readiness.html` | Pass | `npm run qa:chrome:flows` passed the local fixture through the real popup action path; total findings `20`; warnings: `TLS scan unavailable: SSL Labs returned HTTP 441.` and `Certificate scan unavailable: Certificate scan requires an HTTPS page.` |
| Scan `https://www.cloudflare.com/` | Pass | `npm run qa:chrome:flows` passed the known HTTPS scan through the real popup action path; total findings `55`; TLS results rendered; warnings recorded: page data truncation and certificate lookup HTTP 504. |
| Scan `http://example.com/` | Pass | `npm run qa:chrome:flows` passed the HTTP scan through the real popup action path; total findings `4`; visible warnings included TLS unavailable `Hostname blacklisted` and certificate unavailable `Certificate lookup returned HTTP 504.` |
| Attempt unsupported page such as `chrome://extensions/` | Pass | `npm run qa:chrome:flows` confirmed graceful popup error: `Floun can scan HTTP and HTTPS tabs only.` |
| Generate PDF report | Pass | `npm run qa:chrome:flows` downloaded `Quantum_Safe_Cryptography_Report.pdf` from the fixture scan; size `37095` bytes; raw fixture token leaks `0`. |
| Store package built without Gemini key | Pass | `npm run release:artifact` found no Gemini API-key-like values. |

Release default: do not tag, push, upload, or publish until `npm run release:publish:check` passes against this evidence.
